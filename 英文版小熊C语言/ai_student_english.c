...
static void draw_scatter_plot(AppState *state, int x, int y, int w, int h) {
    ...
    DrawText("Scatter: AI Hours vs Final GPA", x + 10, y + 8, 14, COLOR_TEXT);
    ...
    DrawText("Weekly AI Hours", plot_x + plot_w / 2 - 50, plot_y + plot_h + 8, 11, COLOR_TEXT_DIM);
    DrawText("Final GPA", plot_x - 50, plot_y - 2, 11, COLOR_TEXT_DIM);
    ...
    for (int i = 0; i < count; i++) {
        float px = plot_x + plot_w * (data[i].ai_hour - norm->hour_min) / (norm->hour_max - norm->hour_min);
        float py = plot_y + plot_h * (1 - (data[i].gpa_end - norm->gpa_min) / (norm->gpa_max - norm->gpa_min));
    ...
    if (state->current_mode == MODE_KMEANS || state->current_mode == MODE_KNN) {
        for (int c = 0; c < state->km_model.k; c++) {
            float hour_val = state->km_model.centroids[c][2] * (norm->hour_max - norm->hour_min) + norm->hour_min;
            float gpa_val = state->km_model.centroids[c][1] * (norm->gpa_max - norm->gpa_min) + norm->gpa_min;
            
            float cx = plot_x + plot_w * (hour_val - norm->hour_min) / (norm->hour_max - norm->hour_min);
            float cy = plot_y + plot_h * (1 - (gpa_val - norm->gpa_min) / (norm->gpa_max - norm->gpa_min));
    ...
    int legend_x = x + w - 130, legend_y = y + 35;
